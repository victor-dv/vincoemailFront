...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
  ...
  const [user, setUser] = React.useState<{ name: string; email: string; username: string } | null>(null);
  ...
  useEffect(() => {
    ...
    if (token) {
      try {
        ...
        setUser({ name: decoded.name, email: decoded.email, username: decoded.username });
      } catch (error) {
      ...
      `}>
        ...
        <div className="p-4 border-t border-gray-200">
          <div className="flex items-center gap-5 px-3 py-2">
            ...
            <div className="flex flex-col">
              <span className="text-sm font-medium text-gray-900">{user?.username || "Usuário"}</span>
